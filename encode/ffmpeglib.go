...
func Split(input string, seconds int, outputDir string, videoId string, ext string) (*list.List, error){
	...
	" -f segment -strict -2 " + outputDir + "%04d" + ext
...
// GENERATE THUMBNAIL
func GenerateThumbnail(videoFile string, outputDir string , videoId string) (fileNameOut string, err error) {
	//log.Println("-- start generating thumbnail...")
	fileNameOut = videoId + ".png"
	fileDestination := outputDir + fileNameOut

	//[]byte, err
	_, err = exec.Command("ffmpeg", "-i", videoFile, "-t", "0.001", "-ss", "7", "-vframes", "1", "-y", "-f",
...
func CreateM3U8(segmentsDir string, outputDir string, videoId string) error {

	fileName := outputDir + videoId + ".m3u8"
	...
	//Write header
	txtFile.WriteString("#EXTM3U\n")
	txtFile.WriteString("#EXT-X-TARGETDURATION:10\n")
	txtFile.WriteString("#EXT-X-ALLOW-CACHE:NO\n")
	txtFile.WriteString("#EXT-X-VERSION:3\n")
	txtFile.WriteString("#EXT-X-MEDIA-SEQUENCE:1\n")
	...
	for _, segmentName := range files {
		vars := strings.Split(segmentName.Name(), ".")
		name := vars[0]
		//ext := vars[1]
		filePath := segmentsDir + segmentName.Name()
		duration, _ := GetSegmentDuration(filePath)

		durationLine := "#EXTINF:" + duration + "\n"
		locationLine := "/ts/" + videoId + "/" + name + ".ts" + "\n"
		txtFile.WriteString(durationLine)
		txtFile.WriteString(locationLine)
	}
	txtFile.WriteString("#EXT-X-ENDLIST")
	log.Printf("-- created %s \n", videoId +".m3u8")
	return nil
}


func GenerateMediaInfo(videoPath string,videoId string ) (*context.VideoArgs, error) {
	videoFileName := videoPath
	codecVid, codecAud, err := GetCodec(videoFileName)
	if err != nil {
		return nil, errors.New("Media Mining Error:" + err.Error())
	}

	pixelWidth, pixelHeight, err := GetResolution(videoFileName)
	if err != nil {
		return nil, errors.New("FFMPEG Height & Width error:" + err.Error())
	}

	mediaDuration, err := GetDuration(videoFileName)
	if err != nil {
		return nil, errors.New("FFMPEG Media-Duration error:" + err.Error())
	}

	videoUrl := config.STREAM_URL + "/mp4/" + videoId +".mp4"
	thumbnailURL := config.STREAM_URL + "/thumbnail/" + videoId + ".png"
	hslURL := config.STREAM_URL + "/m3u/" + videoId + ".m3u8"

	this := &context.VideoArgs{
		VideoId:      videoId,
		AudioCodec:   codecAud,
		VideoCodec:   codecVid,
